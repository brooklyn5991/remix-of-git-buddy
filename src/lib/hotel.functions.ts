...
// ---------------- Public: verify Paystack, then create a confirmed category booking ----------------
const confirmPaystackBookingSchema = z.object({
  tier: z.enum(["Standard", "Deluxe", "Executive", "Suite"]),
  guest_name: z.string().min(2).max(120),
  guest_email: z.string().email(),
  guest_phone: z.string().min(6).max(30),
...
export const confirmPaystackBookingByTier = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: existing, error: existingErr } = await supabaseAdmin
      .from("reservations")
      .select("id, confirmation_code, guest_name, guest_email, guest_phone, check_in, check_out, total_ngn, room_id, rooms(room_number, tier, name)")
      .eq("payment_reference", data.reference)
      .maybeSingle();
    if (existingErr) throw new Error(existingErr.message);
    if (existing) {
      const existingRoom = existing.rooms as { room_number: string; tier: string; name: string } | null;
      return {
        ok: true,
        reservation: {
          ...
          room_tier: existingRoom?.tier ?? data.tier,
    ...
    const price = Math.min(...tierRooms.map((room) => room.price_ngn as number));
    ...
    const paidCorrectly =
      ...
      body.data.amount === expectedKobo &&
      body.data.currency === "NGN" &&
      body.data.reference === data.reference;
    if (!paidCorrectly) throw new Error("Payment was not successful");

    const roomIds = tierRooms.map((room) => room.id as string);
    ...
    const availableRooms = tierRooms.filter((room) => !bookedIds.has(room.id as string));
    if (availableRooms.length === 0) throw new Error(`All ${data.tier} rooms are sold out for those dates.`);
    ...
    for (const room of availableRooms) {
      ...
      if (!createErr && created) {
        const createdRoom = created.rooms as { room_number: string; tier: string; name: string } | null;
        return {
          ok: true,
          reservation: {
            id: created.id as string,
            ...
            check_out: created.check_out as string,
            total_ngn: created.total_ngn as number,
            room_number: createdRoom?.room_number ?? (room.room_number as string),
            room_tier: createdRoom?.tier ?? data.tier,
            room_name: createdRoom?.name ?? (room.name as string),
          },
        };
      }

      if (createErr && (createErr.code === "23P01" || createErr.message.toLowerCase().includes("exclude"))) {
        continue;
      }

      if (createErr && createErr.code === "23505") {
        const { data: paidReservation } = await supabaseAdmin
          .from("reservations")
          .select("id, confirmation_code, guest_name, guest_email, guest_phone, check_in, check_out, total_ngn, room_id, rooms(room_number, tier, name)")
          .eq("payment_reference", data.reference)
          .maybeSingle();
        if (paidReservation) {
          const paidRoom = paidReservation.rooms as { room_number: string; tier: string; name: string } | null;
          return {
            ok: true,
            reservation: {
              id: paidReservation.id as string,
              confirmation_code: paidReservation.confirmation_code as string,
              guest_name: paidReservation.guest_name as string,
              guest_email: paidReservation.guest_email as string,
              guest_phone: paidReservation.guest_phone as string,
              check_in: paidReservation.check_in as string,
              check_out: paidReservation.check_out as string,
              total_ngn: paidReservation.total_ngn as number,
              room_number: paidRoom?.room_number ?? "",
              room_tier: paidRoom?.tier ?? data.tier,
              room_name: paidRoom?.name ?? "",
            },
          };
        }
      }
